...
export default function DashboardPage() {
  const stats = [
    ...
    ["Authentic Videos", "8", "33.3%", "text-emerald-400"],
    ["On-Chain Records", "24", "100% Secured", "text-purple-400"],
  ];
  return (
    <div className="p-6">
      <Panel className="mb-6">
        <PanelHeader step="07" title="Dashboard" subtitle="Overview of your verification activity" />
        <div className="p-6 grid grid-cols-4 gap-4">
          {stats.map(([label, val, sub, color]) => (
            <div key={label} className="rounded-xl border border-slate-800 p-4" style={{ background: "#0f1120" }}>
      ...
      <div className="grid grid-cols-3 gap-6">
        <Panel className="col-span-2 p-5">
          <div className="flex justify-between mb-4">
            <p className="text-xs font-semibold text-slate-300">Verification Trend</p>
          ...
          <div style={{ width: "100%", height: 200 }}>
            <ResponsiveContainer>
              <LineChart data={TREND_DATA}>
                ...
                <Tooltip contentStyle={{ background: "#11131f", border: "1px solid #1e2233", fontSize: 12 }} />
        ...
        <Panel className="p-5">
          <p className="text-xs font-semibold text-slate-300 mb-4">Result Distribution</p>
          <div style={{ width: "100%", height: 140 }}>
            <ResponsiveContainer>
              <PieChart>
                <Pie data={DIST_DATA} dataKey="value" innerRadius={40} outerRadius={60} paddingAngle={3}>
                  {DIST_DATA.map((d) => <Cell key={d.name} fill={d.color} />)}
                </Pie>
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      <Panel className="mt-6 p-5">
        <div className="flex justify-between mb-4">
          <p className="text-xs font-semibold text-slate-300">Recent Verifications</p>
          <span className="text-[11px] text-purple-400">View all</span>
        </div>
        <ul className="divide-y divide-slate-800">
          {RECENT_VERIFICATIONS.map((v) => (
            <li key={v.name} className="flex items-center justify-between py-2.5 text-xs">
              <div>
                <p className="text-slate-300">{v.name}</p>
                <p className="text-slate-600 text-[10px]">{v.meta}</p>
              </div>
              <span className={v.tone === "rose" ? "text-rose-400" : "text-emerald-400"}>{v.score}</span>
            </li>
          ))}
        </ul>
      </Panel>
